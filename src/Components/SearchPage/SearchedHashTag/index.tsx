...
const SearchedHashTag = ({ searhValue }) => {
  ...
  const getSearchHashTagOrder = async (pageParam: number) => {
  ...
  const getSearchHashTagNewest = async (pageParam: number) => {
  ...
  return (
    <Body>
      ...
      <div>
        {data?.pages.map((page, index: React.Key) => (
          <div key={index}>
            {page?.Post.map((posts: Post, index: React.Key) => (
